...
contract TokenSale is Initializable, ITokenSale {
    ...
    struct State {
        ...
        uint8 tokenDecimals;
        int256 exchangeRate;
        uint256 totalPrivateSold;
        uint256 freePrivateSold;
        uint256 totalPublicSold;
        uint256 totalSupplyDecimals;
        uint256 publicMaxValues;
    }
    ...
    function initialize(
        ...
        state.totalSupplyDecimals = _multiply(_params.totalSupply);
     ...
     */
    function _checkingEpoch() internal {
    ...
    function _onlyAdmin() view internal {
        ...
        msg.sender == address(admin), 'Sender is not an admin');
    }
    /**
    ...
    function deposit() external payable {
        ...
        if (state.exchangeRate == 0) {
            ...
            privatePrice = (params.privateTokenPrice * PCT_BASE) / (uint256(state.exchangeRate) * ORACLE_MUL);
            publicPrice = (params.publicTokenPrice * PCT_BASE) / (uint256(state.exchangeRate) * ORACLE_MUL);
        }
    ...
    function _processPrivate(address _sender, uint256 _amount) internal {
        ...
        uint256 inValue = (maxInFiat * PCT_BASE) / (uint256(state.exchangeRate) * ORACLE_MUL);
        ...
        if (t == 7) {
            if (!s.free) {  
                s.free = true;
                state.freePrivateSold += s.amount != 0 ? s.amount : 0;
            }
            state.freePrivateSold += add;
        }
        state.totalPrivateSold += add;
        s.amount += add;
    ...
    function claim() external override {
        _checkingEpoch();
        require(uint8(epoch) > 1 && !admin.blockClaim(address(this)), "incorrect time");
        address sender = msg.sender;
        require(!claimed[sender], "already claims");
        Staked storage s = stakes[sender];
        require(s.amount != 0, "doesnt have a deposit");
    ...
    function _earned() internal view returns (uint256 earned) {
        ...
        if (soldOut) {
            earned =
                (((totalForSale - state.totalPublicSold) *
                    privatePrice) +
    ...
    function takeLocked() external override{
        _onlyAdmin();
        require(
            block.timestamp >= (params.publicEnd + 2592e3),
            "It is not time yet"
        );
        uint256 amountTkn = token.balanceOf(address(this));
    ...
    function amountForSale() public view returns (uint256) {
        return
            (state.totalSupplyDecimals - _escrowAmount()) -_tokenFee();
    }
